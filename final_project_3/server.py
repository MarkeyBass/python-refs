...
def get_client_data(client):
    try:
        ...
        # When client have problem with file location it disconnects and is removed from the list
        # if err.msg_err == "dealing with FileNotFoundError":
        #     print("Client ({}: {}) disconnected due to file not found error" .format(*client_diction[client]))
        #     del client_diction[client]
        # pattern = r"sent wrong data format!"
        # if err.msg_err == "sent wrong data format!":
        #     print(f"Due to wrong data format and possibility of hacking client {client_diction[client]} is removed from list")
        #     del client_diction[client]
...
# This function creates a json file which is picture of the database
# Initially created before there was a graphical interface to present easily the DB content in a human readable file.
